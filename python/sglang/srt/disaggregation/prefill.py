...
class SchedulerDisaggregationPrefillMixin:
    ...
    def maybe_send_cached_prefix_chunk(self: Scheduler, req: Req) -> None:
        ...
        # The DCP KV relayout transfers the full sequence as one token-granular
        # per-rank plan; the cached-prefix early-send would split the transfer into
        # chunks the relayout cannot reassemble, dropping the newly-computed tail on
        # the decode (so KV-cache-hit requests decode without the fresh suffix). Skip
        # the optional early-send for DCP-sharded decodes; the regular final send
